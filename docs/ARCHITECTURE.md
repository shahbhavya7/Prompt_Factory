...
flowchart RL
    DB[("chunks<br>Postgres + pgvector<br>33 seed + N learned")]
    ...
    subgraph LANE_A["LANE A — live turn: one LLM call, ~1.0-1.3k tokens"]
        ...
        A3{"IntentRouter.detect<br>cosine vs 53 caller-phrased exemplars<br>best label >= 0.45 ?"}
    ...
    subgraph LANE_B["LANE B — post-call learning: between calls only, never on the hot path"]
        ...
        B5{"GATE 2 — duplicate<br>cue cosine vs whole pool<br>> 0.85 ?"}
...
Fourteen modules plus two test files. Dependency direction is strictly one-way:
`models` ← `kb` ← {`db`, `retrieve`, `consolidator`} ← `engine` ← `streamlit_app`.
Nothing in `sace_chat/` imports `streamlit_app`, and `retrieve.py` imports no
...
**Called by:** `kb.py` (constructs 33 of them), `db.insert_chunk`, `retrieve._row_to_chunk`, `consolidator.run_learning_loop`.

---

### `sace_chat/kb.py`

**Purpose.** The hand-authored knowledge base: the stable core, the intent exemplar sets, and the 33 seed rules. Pure data — no logic.
...
| `INTENT_EXEMPLARS` | `dict[str, list[str]]` — 14 labels, 53 short caller-phrased exemplars. The routing table for `IntentRouter`. |
| `RULES` | `list[Chunk]` — 33 rules: **19 general** (`intent=None`) + **14 intent-routed**. 11 are `terminal`, the same 11 are `exclusive`. |
...
**Purpose.** Schema, connection, migration, and the single validated insert path. Owns both tables.
...
| `embedding` | `Vector(EMBEDDING_DIM)` | no | Cosine-searched with pgvector's `<=>`. **Exact sequential scan — no HNSW/IVFFlat index**, deliberately: at 35 rows an approximate index combined with a strict `WHERE` can under-return. |
...
**Called by:** `load_kb.py` and `consolidator.py` both go through `insert_chunk`, so the two can never disagree about which columns get set. `engine.py` and `streamlit_app.py` import the `engine` (SQLAlchemy) object for read connections.

---

### `sace_chat/embeddings.py`

**Purpose.** Two interchangeable embedders behind one interface, plus a batching helper.
...
| `OpenAIEmbedder.dim = 1536` | `text-embedding-3-small`. Key from `OPENAI_API_KEY`, falling back to `SACE_LLM_KEY`. |
| `OpenAIEmbedder.embed(text)` | One embedding, one request. |
| `OpenAIEmbedder.embed_many(texts)` | **One request for the whole batch**, re-sorted by the API's `index` field (order is not promised). Used for the 53 exemplars at boot and the two query vectors per turn — sequentially those are dozens of round-trips of dead air. |
| `embed_many(embedder, texts)` | Module-level: uses `embedder.embed_many` if present, else loops. |
| `get_embedder()` | `EMBEDDING_MODE=openai` → `OpenAIEmbedder`, else `MockEmbedder`. |

**Called by:** `retrieve.py`, `engine.py`, `load_kb.py`, `consolidator.py` (indirectly), `streamlit_app.py`, `scripts/repair_embeddings.py`.

> **`EMBEDDING_DIM` must match the mode.** The vector column's dimension is fixed at table-creation time, so switching mode without matching `EMBEDDING_DIM` produces Postgres's `different vector dimensions 1536 and 384` at query time. `run.sh` warns on the mismatch; `check_embedding` catches it at insert.
...
| `IntentRouter.warm()` | Embeds all 53 exemplars in **one batch**, once. Idempotent. Called explicitly at app boot so it is not paid as first-turn latency. | calls embedder |
...
| Signature | Behaviour |
|---|---|
| `VALID_INTENTS` | 15 labels including `"none"`. Must stay in step with `INTENT_EXEMPLARS`' keys and the rules' `intent` values. |
...
| `resolve_precedence(intent, message) -> (str, bool)` | Returns `(effective_intent, opt_out)`. **abuse outranks dnc** when both read true in one utterance, but the call is still tagged `opt_out`. **A named day/time outranks `busy`.** A bare `dnc` sets `opt_out`. |
...
**Purpose.** Turns a `Retrieval` into the system prompt. Six sections, fixed order.
...
| `_TURN_INSTRUCTION` | 321 tokens: the JSON schema plus seven rules — content comes only from GOVERNING; never re-ask ALREADY ASKED; a hedge is an answer; intent from the fixed label set; the reply is spoken aloud (no JSON, no bracketed tokens); `extracted_fields` only for values actually stated. |
...
| `build_turn_prompt(stable_core, state, retrieval, history, reinforce_reason="") -> str` | Joins the six sections and substitutes placeholders. Returns the **system** prompt; the caller's message is sent separately as the user message. |
...
| `Engine._decide(prompt, user_message, state, notes, sent_log)` | **Captures the payload immediately before the call** via `llm.build_messages` / `render_messages`, asserts the caller message is present, appends to `sent_log`, then calls the LLM, parses, and validates. | calls LLM; appends `sent_log` |
| `Engine.step(state, history, user_message) -> (reply, state, debug)` | The turn. See ordering below. | reads `chunks`; calls embedder + LLM; **mutates `state` and `history`** |
...
| `_rule_debug(rule, scores) -> dict` | Flattens a rule + its score for the UI. | pure |

**`step()` ordering** — the sequence matters:

1. `_retrieve` — one query, current state.
2. `build_turn_prompt` → `_decide` (LLM call #1, payload captured).
...
| `OpenAICompatibleLLM.__init__(api_key=None, base_url=None, model=None)` | `SACE_LLM_KEY`, `SACE_LLM_BASE`, `SACE_LLM_MODEL` (default `gpt-4o-mini`). |
| `OpenAICompatibleLLM._kwargs(messages, json_mode)` | Adds `response_format={"type":"json_object"}` when `json_mode`. **Omits `temperature` for `^(gpt-5\|o\d)`** — reasoning models reject an explicit temperature with a 400. |
...
| `DUPLICATE_THRESHOLD` | `0.85` | See [§5](#5-the-three-verification-gates). |
| `SAME_TOPIC_THRESHOLD` | `0.6` | Below this, two rules are unrelated and "conflict" is meaningless. |
...
| `_fetch_pool(conn, table)` | `SELECT id, text, embedding FROM chunks` — the **whole** pool. One flat memory means a candidate is compared against everything, not one folder. | reads `chunks` |
...
`main()` — `init_db()`, then **delete only `source != 'learned'`** rows and re-insert all 33 seed rules via `insert_chunk`. Learned rules survive a reload by construction. Prints seed/general/intent-routed counts, how many rows were replaced, and how many learned rules were preserved.
...
| **Real embeddings, not the mock** | `MockEmbedder` — 384-dim hashed bag-of-words. | Hashed word buckets encode vocabulary overlap, not meaning, so paraphrases score no better than unrelated text. Every threshold in the system (0.45 intent, 0.45 grounding, 0.85 duplicate, 0.6 same-topic) is meaningless under it. | `text-embedding-3-small` (1536-dim) for anything measured. The mock is retained only so the pipeline and UI run with no API key. |
| **Semantic intent detection, not regex** | `detect_intent` — an ordered list of regex patterns per label. | Missed every unanticipated phrasing. `"is this even worth it vs my work plan"` is a pricing question with no pricing keyword in it. Each miss cascaded into wrong retrieval and then a hallucinated reply. | `IntentRouter`: cosine against 53 caller-phrased exemplars, threshold 0.45. Regex survives only inside `mentions_day_or_time`, answering one narrow factual question for a precedence rule. |
...
### Gate 2 — Duplicate (`DUPLICATE_THRESHOLD = 0.85`)

The candidate's **cue** is embedded and compared by cosine against every rule in the pool. Above 0.85 the candidate is dropped as `duplicate-skipped`, recording which rule it matched and at what cosine.

*Why 0.85 and not the textbook 0.9:* a short extracted candidate restating a long, dense hand-authored rule scores lower than a same-length duplicate would, because cosine is sensitive to length asymmetry. 0.85 was verified against this KB's actual rule lengths and is still a strict bar for "clearly the same rule".
...
Only candidates that are **related but not duplicates** (cosine between 0.6 and 0.85) are tested for contradiction. `_is_conflict` fires when the two texts both mention numbers and the number *sets* differ (a changed callback number, different opening hours), or when one asserts what the other denies. On a hit, the candidate goes to `needs_review` with `reason="conflict"` **and the id of the rule it contradicts**.
...
- **Duplicate and conflict thresholds have tight margins.** 0.85 and 0.6 were tuned against `text-embedding-3-small` and against *this* KB's rule lengths. Short-candidate-vs-long-rule comparisons are the weak case: cosine is length-sensitive, so a genuine duplicate expressed tersely can score below 0.85 and be inserted. One such near-duplicate of `special_dnc` sits in the pool today at a cosine below threshold. The thresholds do not transfer to a different embedding model or a KB with materially different rule lengths without re-tuning.
...
- **No index on `embedding`, by design.** Exact sequential scan is correct and fast at 35 rows. The flat per-turn prompt cost is independent of pool size, but *retrieval* is O(pool) and will need an ANN index long before the prompt size becomes a problem — at which point the interaction between an approximate index and a strict `WHERE intent = …` needs care, since that combination can under-return.
...
- **Single-tenant, single-call state.** `CallState` lives in Streamlit session state; the learned-rule pool is global and shared. There is no per-caller memory, no concurrency control on `insert_chunk`, and no tenancy boundary.
